    ...
    case 'radio-group':
      {
        ...
        if (fieldData.type === 'checkbox') {
          ...
          $('input[id$="preview"]').prop('disabled', true);
        }
    ...
    case 'autocomplete':
    {
        ...
        if (fieldOptions) {
                  ...
                  for (var _i = 0; _i < fieldOptions.length; _i++) {
                    ...
                    optionAttrs.id = fieldData.id + '-' + _i;
                    _optionAttrsString = fbUtils.attrString(optionAttrs);
                   optionsMarkup += '<label for="' + optionAttrs.id + '">' + optionAttrs.label + '</label> <input type = "text"' + _optionAttrsString + ' /> <br>';
...
function formBuilderHelpersFn(opts, formBuilder) {
  ...
  _helpers.xmlSave = function (form) {
    ...
    utils.forEach(formData, function (fieldIndex, field) {
      ...
      // Handle options
      if (field.type.match(/(select|checkbox-group|radio-group|autocomplete)/)) {
      ...
      utils.forEach(form.childNodes, function (index, field) {
        ...
        if (!$field.hasClass('disabled')) {
          ...
          (function () {
            ...
            multipleField = fieldData.type.match(/(select|checkbox-group|radio-group|autocomplete)/);
  ...
  _helpers.updatePreview = function (field) {
    ...
    if (fieldType.match(/(select|checkbox-group|radio-group|autocomplete)/)) {
      previewData.values = [];
...
$('input[id$="preview"]').prop('disabled', true);
    ...
    var advFields = function advFields(values) {
      var advFields = [],
          key,
          optionFields = ['select', 'checkbox-group', 'radio-group', 'autocomplete'],
      ...
      if (values.type === 'checkbox-group' || values.type === 'radio-group' || values.type === 'autocomplete') {
    ...
    function inputUserAttrs(name, attrs) {
      ...
      if (!utils.inArray(textAttrs.type, ['checkbox', 'checkbox-group', 'radio-group', 'autocomplete'])) {
...
/*###########################################
#######Remove the pre-selected element#######
*/
    ...
    $sortableFields.on('change', '.prev-holder input, .prev-holder select', function (e) {
      ...
      if (utils.inArray(field.type, ['select', 'checkbox-group', 'radio-group', 'autocomplete'])) {
